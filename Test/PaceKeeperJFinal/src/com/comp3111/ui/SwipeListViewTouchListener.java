...
public class SwipeListViewTouchListener implements View.OnTouchListener {
	...
	private boolean enableLeftSwipe = true;
	private boolean copeWithViewpager = false;
	...
	public boolean onTouch(View view, MotionEvent motionEvent) {
		...
		case MotionEvent.ACTION_DOWN: {
			...
			float tempX = motionEvent.getRawX();
			
			if (copeWithViewpager && tempX > mListView.getWidth() * 4 / 7 + listViewCoords[0]) {
				view.onTouchEvent(motionEvent);
				return true;
			}

			int x = (int) tempX - listViewCoords[0];
		...
		case MotionEvent.ACTION_UP: {
			...
			if (Math.abs(deltaX) > mViewWidth / 2) {
				if(enableLeftSwipe || deltaX > 0){
					swipe = true;
					swipeRight = deltaX > 0;
				}
			} else if (mMinFlingVelocity <= velocityX
					&& velocityX <= mMaxFlingVelocity && velocityY < velocityX) {
				float tempXVel= mVelocityTracker.getXVelocity();
				if(enableLeftSwipe || tempXVel > 0 ){
				swipe = true;
				swipeRight = tempXVel > 0;
				}
			}
			if (swipe) {
