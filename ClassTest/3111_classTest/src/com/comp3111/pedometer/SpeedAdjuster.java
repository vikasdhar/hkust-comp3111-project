...
public class SpeedAdjuster extends PedometerSettings{
	...
	public static int react(Pedometer pedo_obj){	//, final SoundTouchPlayable st_obj){
		...
		if(!saChanging){
			...
			if(pedo_obj.getAverageStepDuration() < saStepDurationLowerThreshold){
				saState = SA_FAST;
				if(saState != saLastState)	rampToSpeed( SA_FAST);
				return SA_FAST;
			}else if(pedo_obj.getAverageStepDuration() > saStepDurationUpperThreshold){
				saState = SA_SLOW;
				if(saState != saLastState)	rampToSpeed( SA_SLOW);
				return SA_SLOW;
			}else{
				saState = SA_NORMAL;
				if(saState != saLastState)	rampToSpeed( SA_NORMAL);
				return SA_NORMAL;
			}
		}
	...
	public static void rampToSpeed(final int speed_level){	//(final SoundTouchPlayable st_obj, final int speed_level){
